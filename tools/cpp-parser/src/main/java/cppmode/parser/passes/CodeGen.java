...
public final class CodeGen {
    ...
    private static void emitComments(StringBuilder sb, List<Token> comments, int depth) {
    ...
    private static void emitVariableDecl(StringBuilder sb, VariableDecl vd, int depth) {
        ...
        if (vd.isConst() && !(vd.type() instanceof NamedType nt && nt.isConst())) {
            // Only prepend "const" here if the TypeRef itself doesn't
            // already carry isConst (NamedType.isConst, rendered by
            // renderTypeAndName/renderTypeRef as a "const " prefix on the
            // type itself) -- avoids ever emitting "const const Type" for
            // a declaration where both flags happen to be set.
            sb.append("const ");
        }
