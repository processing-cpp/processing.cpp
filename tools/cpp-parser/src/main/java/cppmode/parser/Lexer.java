...
/**
 * Tokenizes CppMode source text (Java-flavored C++) into a flat list of {@link Token}s.
 *
...
public final class Lexer {
    ...
    private Token lexPreprocessorDirective(int startLine, int startCol) {
        ...
        return new Token(TokenType.PREPROCESSOR_DIRECTIVE, sb.toString(), startLine, startCol);
    }

    private Token lexLineComment(int startLine, int startCol) {
        StringBuilder sb = new StringBuilder();
        // consume "//"
        sb.append(advanceChar());
        sb.append(advanceChar());
    ...
    private Token lexBlockComment(int startLine, int startCol) {
        StringBuilder sb = new StringBuilder();
        sb.append(advanceChar()); // /
        sb.append(advanceChar()); // *
        while (pos < len) {
    ...
    private Token lexStringLiteral(int startLine, int startCol) {
        StringBuilder sb = new StringBuilder();
    ...
    private Token lexCharLiteral(int startLine, int startCol) {
        StringBuilder sb = new StringBuilder();
        sb.append(advanceChar()); // opening '
        while (pos < len && peekChar() != '\'') {
            if (peekChar() == '\\' && pos + 1 < len) {
    ...
    private Token lexNumericLiteral(int startLine, int startCol) {
        StringBuilder sb = new StringBuilder();
    ...
    private Token lexIdentifierOrKeyword(int startLine, int startCol) {
        ...
        if (text.equals("true") || text.equals("false")) {
            return new Token(TokenType.BOOL_LITERAL, text, startLine, startCol);
        }
    ...
    private Token lexSymbol(int startLine, int startCol) {
        ...
        for (String sym : MULTI_CHAR_SYMBOLS) {
            if (matchesAhead(sym)) {
                ...
                return new Token(symbolTokenType(sym), sym, startLine, startCol);
            }
        }
        char c = peekChar();
        if (SINGLE_CHAR_SYMBOLS.contains(c)) {
            advanceChar();
            return new Token(symbolTokenType(String.valueOf(c)), String.valueOf(c), startLine, startCol);
        }
        ...
        advanceChar();
        return new Token(TokenType.PUNCTUATION, String.valueOf(c), startLine, startCol);
    }
    ...
    private TokenType symbolTokenType(String sym) {
        return PUNCT_SYMBOLS.contains(sym) ? TokenType.PUNCTUATION : TokenType.OPERATOR;
    }
}
