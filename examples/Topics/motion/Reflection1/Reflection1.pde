...
// A fixed-size array of subpoints along the floor path, re-created
// (not resized) each time createGround() runs, matching Java's own
// "arrays can't resize, only be reassigned" behavior. Array<T> has no
// default constructor, so it starts at size 0 here and is properly
// sized the moment setup() calls createGround().
Array<PVector> coords(0);
...
void createGround() {
  ...
  coords = Array<PVector>((int)ceil(baseLength));
  for (int i=0; i<coords.length(); i++) {
