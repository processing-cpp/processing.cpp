...
public final class CodeGen {
    ...
    private static String renderTypeAndName(TypeRef type, String name) {
        if (type instanceof FunctionPointerType fpt) {
            ...
            // Detect array-of-fn-ptr via "__arr__" sentinel added by parser.
            ...
            if (ptaSentinel >= 0) {
                ...
                if (dimIdx >= 0) {
                    ...
                    // No sentinel -- must be ptr-to-array (outer dims)
                    outerDims = dims;
                }
            }
    ...
    private static void emitFunctionDecl(StringBuilder sb, FunctionDecl fd, int depth) {
        indent(sb, depth);
        if (fd.templateParams() != null && (fd.templateParams().isEmpty()
                ? fd.name().contains("::") && fd.name().contains("<") // explicit specialization: template<>
                : true)) {
            sb.append("template<");
            for (int i = 0; i < fd.templateParams().size(); i++) {
                if (i > 0) sb.append(", ");
                sb.append(fd.templateParams().get(i));
            }
            sb.append(">\n");
            indent(sb, depth);
        }
    ...
    private static void emitStmt(StringBuilder sb, Statement s, int depth) {
        ...
        if (s instanceof Block b) {
            ...
            indent(sb, depth);
            sb.append(r.isCoReturn() ? "co_return" : "return");
