...
public final class Parser {
    ...
    private List<TopLevelItem> parseTopLevelItem(List<CppLexerToken> leadingComments) {
        ...
        // "extern template class Foo<T>;" -- explicit instantiation declaration
        // Suppresses implicit instantiation in this TU. Consume verbatim to ";".
        if (checkKeyword("extern") && pos + 1 < tokens.size()
                && tokens.get(pos + 1).isKeyword("template")) {
            int startPos = pos;
            ...
            raw.append(';');
            return List.of(new PreprocessorLine(raw.toString(),
                tokens.get(startPos).line(), tokens.get(startPos).col(), leadingComments));
        }
        if (checkKeyword("extern") && pos + 1 < tokens.size()
    ...
    private String parseTemplateParamName() {
        ...
        // --- concept-constrained type parameter: "Numeric T" or "Numeric typename" ---
        // Also handles concept-constrained template-template params:
        // "template<Concept typename> class C" -- the inner param has
        // concept name then "typename" keyword (not an identifier).
        if (check(CppLexerTokenType.IDENTIFIER) && pos + 1 < tokens.size()
                && (tokens.get(pos + 1).type() == CppLexerTokenType.IDENTIFIER
                    || tokens.get(pos + 1).isKeyword("typename")
                    || tokens.get(pos + 1).isKeyword("class"))
                && pos + 2 < tokens.size()
                && (tokens.get(pos + 2).isOp(">") || tokens.get(pos + 2).isOp(">>")
                    || tokens.get(pos + 2).isPunct(",")
                    || tokens.get(pos + 2).isPunct("...")
                    || tokens.get(pos + 2).isOp("="))) {
            advance(); // concept name
            if (checkPunct("...")) advance();
            // param name may be a keyword (typename/class) or identifier or absent
            String name;
            if (check(CppLexerTokenType.IDENTIFIER)) name = advance().text();
            else if (checkKeyword("typename") || checkKeyword("class")) { advance(); name = "<anon>"; }
            else name = "<anon>";
    ...
    private List<TopLevelItem> parseTypeDef(List<CppLexerToken> leadingComments, List<String> templateParams) {
        ...
        consumeAttributes();
        // GCC __attribute__((...)): "struct __attribute__((packed)) Name"
        ...
        // MSVC __declspec(...): "struct __declspec(align(16)) Name"
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("__declspec")) {
            advance();
            if (checkPunct("(")) { advance(); int _ad=1; while(!isAtEnd()&&_ad>0){if(checkPunct("("))_ad++;else if(checkPunct(")"))_ad--;advance();} }
        }
    ...
    private boolean looksLikeTopLevelDeclarationOrFunction() {
        ...
        try {
            ...
            matchKeyword("constinit");
            matchKeyword("inline");
            matchKeyword("constexpr"); // "inline constexpr int x" -- C++17 inline variable
            matchKeyword("consteval");
            matchKeyword("constinit");
    ...
    private String parseFunctionOrVariableName() {
        ...
        if (checkOp("<") && looksLikeTemplateArgList()) {
            ...
            sb.append(">");
            name = sb.toString();
            // Qualified member after template specialization: "Class<T>::method"
            // e.g. "void Printer<float>::print()" -- out-of-class specialization.
            while (checkPunct("::")) {
                advance();
                if (checkKeyword("operator")) {
                    name += "::" + parseFunctionOrVariableName();
                } else if (check(CppLexerTokenType.IDENTIFIER)) {
                    name += "::" + advance().text();
                    if (checkOp("<") && looksLikeTemplateArgList()) {
                        StringBuilder sb2 = new StringBuilder(name).append("<");
                        advance(); int d2 = 1;
                        while (!isAtEnd() && d2 > 0) {
                            if (checkOp("<")) { d2++; sb2.append(advance().text()); }
                            else if (checkOp(">")) { d2--; if (d2>0) sb2.append(advance().text()); else advance(); }
                            else sb2.append(advance().text());
                        }
                        sb2.append(">"); name = sb2.toString();
                    }
                }
            }
        }
    ...
    private TopLevelItem parseNamespaceDecl(List<CppLexerToken> leadingComments) {
        CppLexerToken start = peek();
        ...
        if (matchOp("=")) {
            ...
            // Emit verbatim as a PreprocessorLine so CodeGen passes it through unchanged.
            // CppBuild hoists it to global scope (preNs) so it resolves correctly.
            // Space before and after "=" so CppBuild's " = " check fires correctly.
            StringBuilder alias = new StringBuilder("namespace ").append(name).append(" = ");
            while (!isAtEnd() && !checkPunct(";")) {
                alias.append(peek().text());
                // Add space after "::" and between tokens for readability
                // (critical: " = " must be present for CppBuild routing)
                advance();
                if (!isAtEnd() && !checkPunct(";")) alias.append(" ");
            }
            // Trim trailing space before ";"
            String aliasStr = alias.toString().stripTrailing() + ";";
            matchPunct(";");
            return new PreprocessorLine(aliasStr, start.line(), start.col(), leadingComments);
        }
        expectPunct("{");
        List<TopLevelItem> items = new ArrayList<>();
        while (!checkPunct("}")) {
    ...
    private TypeRef parseTemplateArg() {
        ...
        if (check(CppLexerTokenType.INT_LITERAL) || check(CppLexerTokenType.FLOAT_LITERAL)
                ...
                || check(CppLexerTokenType.STRING_LITERAL)) {
            String val = advance().text();
            return new NamedType(val, List.of(), 0, false, false, false);
        }
        ...
        // Parenthesized NTTP expression: "(true ? 4 : 8)", "(sizeof(T) > 2 ? 8 : 4)"
        // The whole parenthesized expression is a valid template argument.
        // Consume it by tracking paren depth and return verbatim as a NamedType.
        if (checkPunct("(")) {
            int pStart = pos;
            advance(); int pd = 1;
            StringBuilder pexpr = new StringBuilder("(");
            while (!isAtEnd() && pd > 0) {
                if (checkPunct("(")) { pd++; pexpr.append("("); advance(); }
                else if (checkPunct(")")) { pd--; if (pd > 0) pexpr.append(")"); advance(); }
                else { pexpr.append(peek().text()); advance(); }
            }
            pexpr.append(")");
            return new NamedType(pexpr.toString(), List.of(), 0, false, false, false);
        }
        ...
        // co_await as expression: "int x = co_await awaitable;"
        // co_await and co_yield can appear as unary prefix expressions,
        // not just as statements.
    ...
    private NameAndFunctionPointerType parseFunctionPointerDeclaratorTail(TypeRef returnType) {
        ...
        // Encode dims into name with "__arr__" sentinel so CodeGen can distinguish
        // array-of-fn-ptr from ptr-to-array (which also has empty paramTypes).
        // e.g. "ops[2]" -> "ops__arr__[2]" -- CodeGen strips sentinel, puts inside parens.
        ...
        if (arrayDimStr.length() > 0) {
            encodedName = encodedName + "__arr__" + arrayDimStr.toString();
        }
        expectPunct(")");
        ...
        // Consume optional noexcept after const: "void (T::*)(int) const noexcept"
        if (checkKeyword("noexcept")) {
            advance();
            if (checkPunct("(")) { advance(); int _nd=1; while(!isAtEnd()&&_nd>0){if(checkPunct("("))_nd++;else if(checkPunct(")"))_nd--;advance();} }
        }
    ...
    private boolean looksLikeCast() {
        ...
        try {
            ...
            // Consume optional array declarator: "(int[])" -- compound literal cast
            while (checkPunct("[")) {
                advance();
                while (!isAtEnd() && !checkPunct("]")) advance();
                if (!matchPunct("]")) return false;
    ...
    private boolean canStartExpression(CppLexerToken t) {
        ...
        if (t.isPunct("(") || t.isPunct("{")) return true;
        if (t.isOp("-") || t.isOp("+") || t.isOp("*") || t.isOp("!") || t.isOp("&")
                || t.isOp("~") || t.isOp("++") || t.isOp("--")) return true;
        // Keyword unary operators that can follow a C-style cast
        if (t.isKeyword("new") || t.isKeyword("delete")) return true;
        if (t.isKeyword("sizeof") || t.isKeyword("alignof") || t.isKeyword("decltype")) return true;
        if (t.isKeyword("throw")) return true;
        if (t.isKeyword("true") || t.isKeyword("false") || t.isKeyword("nullptr")) return true;
        return false;
    }
    ...
    private Expr parseCast() {
        ...
        // Consume optional array declarator in compound literal cast: "(int[]){...}"
        // The [] makes this a C99 compound literal (GCC extension in C++).
        // Bump pointer depth to represent the array-decayed pointer type.
        while (checkPunct("[")) {
            advance();
            if (!checkPunct("]")) parseExpr(); // optional size expression
            expectPunct("]");
            if (targetType instanceof NamedType nt) {
                targetType = new NamedType(nt.baseName(), nt.templateArgs(),
    ...
    private Expr parsePrimary() {
        ...
        // co_await/co_yield as expressions: "int x = co_await awaitable;"
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("co_await")) {
            CppLexerToken coTok = advance();
            Expr coVal = parseUnary();
            return new UnaryExpr("co_await ", coVal, coTok.line(), coTok.col(), List.of());
        }
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("co_yield")) {
            CppLexerToken coTok = advance();
            Expr coVal = parseUnary();
            return new UnaryExpr("co_yield ", coVal, coTok.line(), coTok.col(), List.of());
        }
    ...
    private List<Statement> parseStatementOrMultiDecl(List<CppLexerToken> leadingComments) {
        ...
        if (checkKeyword("using") && pos + 1 < tokens.size()) {
            // using X = Type;  (alias)
            // using X::Y;      (qualified name import, e.g. "using std::string;")
            // using namespace X; (handled earlier)
            // Consume everything to ";" and emit verbatim as an identifier statement.
            int _us = pos;
            while (!isAtEnd() && !checkPunct(";")) advance();
            matchPunct(";");
    ...
    private Statement parseStatement(List<CppLexerToken> leadingComments) {
        ...
        if (checkKeyword("throw")) {
            ...
            if (checkPunct(";")) { advance(); return new ReturnStatement(null, false, t0.line(), t0.col(), leadingComments); }
            Expr val = parseExpr();
            expectPunct(";");
        ...
        // co_return: like return but for coroutines -- may have no operand
        // co_yield:  suspends the coroutine and yields a value
        // co_await:  suspends until the awaitable is ready
        // All three are statement-level in practice; co_yield/co_await can
        // also appear as expressions but handling them as statements covers
        // all real corpus cases.
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("co_return")) {
            CppLexerToken t = advance();
            Expr val = null;
            if (!checkPunct(";")) val = parseExpr();
            expectPunct(";");
            return new ReturnStatement(val, true, t.line(), t.col(), leadingComments);
        }
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("co_yield")) {
            CppLexerToken t = advance();
            Expr val = checkPunct(";") ? null : parseExpr();
            expectPunct(";");
            return new ExprStatement(
                new UnaryExpr("co_yield ", val != null ? val
                    : new Literal(Literal.Kind.INT, "0", t.line(), t.col(), List.of()),
                    t.line(), t.col(), List.of()),
                t.line(), t.col(), leadingComments);
        }
        if (check(CppLexerTokenType.IDENTIFIER) && peek().text().equals("co_await")) {
            CppLexerToken t = advance();
            Expr val = parseExpr();
            expectPunct(";");
            return new ExprStatement(
                new UnaryExpr("co_await ", val, t.line(), t.col(), List.of()),
                t.line(), t.col(), leadingComments);
        }
    ...
    private Statement parseReturn(List<CppLexerToken> leadingComments) {
        ...
        expectPunct(";");
        return new ReturnStatement(value, false, start.line(), start.col(), leadingComments);
    }
    ...
    private static final java.util.Set<String> STATEMENT_KEYWORDS = java.util.Set.of(
        ...
        "try", "delete", "case", "default", "else", "throw", "using"
    );
